...
<head>
    ...
    if (isset($_POST['order']) &&isset($_COOKIE['username']))
    {
...
//    echo $order_id;
        ...
        // echo $product_id;
        $qrystr=" INSERT INTO order_detailes (product_id, order_id, quantity,  username) VALUES ('$product_id', '$order_id', ' $quant ', '$username');";
        $res=$db->query($qrystr);
        $db->close();
        ?>
        <script>
...
<section class="brand_section layout_padding2">
    ...
    <div class="container-fluid brand_item-container">
        ...
        $db=new mysqli('localhost','root','','sugarbloom');
        $qrystr="SELECT * FROM `products` WHERE section ='Donuts';";
        $res=$db->query($qrystr);
        ...
        if($res->num_rows>0)
        {
            ...
            for($i=0;$i<$res->num_rows;$i++){
                ...
                echo "<form action='Donutsuser.php' method='post'> ";
...
//            echo $row->ID;
                ...
                echo "<label for = 'quantity' style='margin-right: 10px;'>Quantity</label> <input id='quantity' name='quantity' type='number' min='1' max='100' value='1' > ";
                echo "</input>";
                echo "<div>";
                echo "<input class='order' type='submit'  name='order' value='Add To Cart'  >";
